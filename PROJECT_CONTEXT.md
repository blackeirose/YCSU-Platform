...
## MAIN Writing v1 (2026-09-18)
Implementation: independent public Markdown content in content/writing, static media in public/writing, one Writing card alongside Products (excluded from Product counts), compact header and native frosted reader. Desktop/tablet use index + reader; mobile uses list + reader. Stable generated routes /writing/<slug>/ have static body/metadata and JS history enhancement. Production begins with zero approved articles; no demo content is released.
Presentation settings and mixed home order use the new main_presentation singleton in the existing Supabase project, with dedicated writing-ops and exact existing owner Auth. Home order, article order and featured state are independent. Existing Registry schema/handler, protected Product links and six-digit OTP remain unchanged. Owner enters Arrange explicitly; Writing settings do not edit Markdown body.
Publishing contract/schema: docs/WRITING.md. UI decisions: DESIGN.md. Exact QA, Core revision and deployment evidence: docs/releases/main-writing-v1.md. Released and production verified at source 2b2cdac08cbf0e5e2db0cff917778a147d70b0c6, Netlify 6aad8e62321b9fa6785b09b8 (2026-09-18). All 28 tests and independent source review passed. Production has 11 Products plus one empty Writing card; owner order save/reload/restore and guest/logout checks passed.

## Writing v1.1 - released 2026-09-18
The first approved article is Make the Impossible Possible (/writing/make-the-impossible-possible/), released at source15a6100 / Netlify6aada2062630da334a6f46db. Earlier zero-article notes describe v1's initial state.
Writing v1.1 adds contextual owner editing of existing canonical Markdown and images through writing-content; the Product Registry, Auth/SMTP and main_presentation schema remain unchanged. Owner management is inside the Writing Viewer; homepage retains only Arrange cards. Public runtime reads current Git content with static deployed fallback; static SEO refresh remains an attended release. New server-only secret MAIN_WRITING_GITHUB_TOKEN was configured by the owner and its name confirmed without reading the value. See DEC-021, docs/WRITING.md and docs/releases/main-writing-v1-1.md for release status.

Production v1.1 polish is verified at Netlify 6aadc5abb7561cd759f6c1e3, source 6bcde87. Public GitHub reads do not depend on the editor token. Actual owner read, canonical image save/readback, no-op save and logout passed before/after publication. All 30 runtime assets match; Registry and presentation payloads are unchanged. No owner action remains. See the release record for limits and recovery.
...
## Writing v1.3.2 — MAIN cover and default article order
Released at merge e1ed82e, Netlify6aaf1e77b3d3a8598605e033. Owner Set as MAIN cover uses existing featured_slug, shows one owner-only MAIN COVER indicator and a disabled selected action. Card content comes from that article; empty/invalid/removed selection falls back to newest date/slug. Unlisted articles sort newest-first before manual-ranked items, preserving manual relative order. No new settings/schema/backend or reader redesign. Full79 tests and independent review PASS; production13 browser/8 API security/35 file hashes PASS; QA settings restored. See docs/releases/main-writing-v1-3-2-cover-order.md. Owner action NONE.
