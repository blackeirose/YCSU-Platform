...
## Product lifecycle
YSU Small Project UI Standard1.0 remains applied with existing MAIN tokens. Product cards share620px height so Hide never shifts following rows; long full content uses native scrolling inside the card. Owner Hidden content opacity0.5, with full-opacity HIDDEN label and controls. Manage menu and recovery sections use44px controls; Delete confirmation is cancel-focused. Guest Hidden has no image/link/actions, only name and UNAVAILABLE. Writing sizing remains independent. Desktop/mobile real-browser acceptance is recorded separately in release notes.
