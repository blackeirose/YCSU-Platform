...
export function createWritingViewer(){
 let ready=false,articles=[],settings={},opener=null,oldOverflow='',lastRoute='';
 ...
 function close(){
  if(!dialog.open)return;dialog.close();document.body.style.overflow=oldOverflow;lastRoute='';document.title='YCSU Platform — Product Registry';
 ...
 return {setData(next,nextSettings){ready=true;articles=next;settings=nextSettings;render()},open(){const a=featuredArticle(articles,settings);navigate('/writing/'+(a?a.slug+'/':''))}};
}
