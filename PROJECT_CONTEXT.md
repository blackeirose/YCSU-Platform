...
The v1.1 candidate adds contextual owner editing of existing canonical Markdown and images through writing-content; the Product Registry, Auth/SMTP and main_presentation schema remain unchanged. Owner management is inside the Writing Viewer; homepage retains only Arrange cards. Public runtime reads current Git content with static deployed fallback; static SEO refresh remains an attended release. New server-only secret MAIN_WRITING_GITHUB_TOKEN was configured by the owner and its name confirmed without reading the value. See DEC-021, docs/WRITING.md and docs/releases/main-writing-v1-1.md for release status.
