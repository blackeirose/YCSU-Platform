...
export function setupWriting({url,key,getOwner,onChange,onAvailability}){
 ...
 async function setFeatured(slug){if(!owner||revision===null)return;const g=generation,e=++editorGeneration;message('Saving featured article…');try{await save('settings',{featured_slug:slug});if(valid(g,e))notify()}catch{if(valid(g,e))message('Could not confirm the save. Reload to check the featured article.')}}
