...
const newestFirst=(a,b)=>b.date.localeCompare(a.date)||a.slug.localeCompare(b.slug);
export function articleOrder(articles,settings=defaults){
 const ranks=new Map((settings.article_order||[]).map((slug,i)=>[slug,i]));
 const unlisted=articles.filter(a=>!ranks.has(a.slug)).sort(newestFirst);
 const listed=articles.filter(a=>ranks.has(a.slug)).sort((a,b)=>ranks.get(a.slug)-ranks.get(b.slug));
 return [...unlisted,...listed];
}
export function featuredArticle(articles,settings=defaults){
 return articles.find(a=>a.slug===settings.featured_slug)||[...articles].sort(newestFirst)[0]||null;
}
...
export function writingCard(articles,settings=defaults){
 const article=featuredArticle(articles,settings),cover=safeUrl(article?article.cover:settings.cover,{media:true});
 ...
 <div class="preview writing-cover">${cover?`<img data-writing-image="${escapeHtml(cover)}" alt="${escapeHtml(article?.cover_alt?.trim()||title)}" width="1440" height="900" loading="lazy">`:''}<span class="writing-placeholder">${article?'YCSU · WRITING':'A space for ideas.'}</span></div>
 ...
 <p class="tagline writing-excerpt">${escapeHtml(article?(article.excerpt||''):settings.description)}</p>
